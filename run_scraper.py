...
NLP_FOMC_RAW_DDL = """
CREATE TABLE IF NOT EXISTS nlp_fomc_raw (
...
cursor = conn.cursor()
cursor.execute(NLP_FOMC_RAW_DDL)
cursor.execute("CREATE INDEX IF NOT EXISTS idx_nlp_fomc_raw_date ON nlp_fomc_raw (release_date)")
conn.commit()
conn.close()
...
logger.info("Step 1a: Calendar page...")
...
for year in range(2007, 2021):
    logger.info(f"Step 1b: Historical {year}...")
...
for idx, link in enumerate(unique, 1):
    ...
    url = link['url']
    logger.info(f"  [{idx}/{len(unique)}] {rd}")
    html = fetch_url(url)
...
logger.info(f"Done! Saved {saved}/{len(unique)}")
...
row = cursor.fetchone()
conn.close()
logger.info(f"DB: {row[0]} records, {row[1]} ~ {row[2]}")
