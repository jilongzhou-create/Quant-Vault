#!/usr/bin/env python3
"""
Framework OOS Backtest - Core+Satellites vs V7 vs Buy&Hold

验证新框架在 OOS 期间 (2020-2026) 的表现。
"""
...
def main():
    ...
    print("\n" + "=" * 78)
    print("  V7 vs Core+Satellites OOS Comparison (2020-2026)")
    print("=" * 78)
    metrics = [
    ...
    print("\n[Step 4] Framework Signal Transparency:")
    df = df_fw.copy()
    for year in sorted(df.index.year.unique()):
        ...
        cp_wt = yr['credit_panic_weight'].mean() if 'credit_panic_weight' in yr.columns else 0
        sp_wt = yr['sge_premium_weight'].mean() if 'sge_premium_weight' in yr.columns else 0
        print(f"  {year}: Gold {yr_return:+.1%}  core={avg_core:+.3f}  "
              f"total={avg_total:+.3f}  exp={avg_exp:.3f}  "
              f"trend={trend_pct:.1%}  CP={cp_trig:.1%}(w={cp_wt:.3f})  "
              f"SP={sp_trig:.1%}(w={sp_wt:.3f})  regime={top_regime}")

    print("\n[Step 5] Key OOS Year Deep Dive (2020, 2022, 2024):")
    ...
    print("\n" + "=" * 78)
    print("  Core + Satellites OOS Backtest Complete!")
    print("=" * 78)
