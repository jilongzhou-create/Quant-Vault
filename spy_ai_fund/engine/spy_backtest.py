#!/usr/bin/env python3
"""
Macro Backtest Engine - SPY 敞口回测引擎

专为"叙事驱动/目标敞口范式"设计的回测引擎。
支持连续敞口滑行 + 现金管理收益 (DTB3 无风险利率)。
...
核心铁律:
  ...
  5. 纯多头: target_exposure ∈ [0.0, 1.0]，绝不裸做空
  6. 数据复权: SPY 价格必须使用 adjClose
"""
...
class SpyBacktestEngine:
    ...
    @staticmethod
    def print_report(result, title="SPY Backtest Report"):
        """打印格式化的绩效报告"""
        print("\n" + "=" * 78)
        print(f"  {title}")
        print("=" * 78)
