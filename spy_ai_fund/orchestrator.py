#!/usr/bin/env python3
"""
SPY AI Fund - Orchestrator (SPY Macro AI Fund)

Pipeline:
  DataEngineer -> FactorMiner -> Gatekeeper -> OOS Autopsy -> MLSynthesizer
       ^                                                          |
       |                Feedback loop: new data requirements       |
       +----------------------------------------------------------+

Iron rules:
  Phase 3 (IS audit): Factor logic and thresholds ONLY based on IS data
  Phase 3.5 (OOS autopsy): IS-passed code FROZEN, OOS fail = DEAD_OOS permanently
  Phase 4 (ML synthesis): Only use factors passing IS + OOS dual verification
"""
...
from spy_ai_fund.agents.data_engineer import DataEngineer
from spy_ai_fund.agents.factor_miner import FactorMiner, load_factor_instance
from spy_ai_fund.agents.gatekeeper import Gatekeeper
...
class Orchestrator:
    ...
    def run_cycle(self, directions: list = None,
                  methods: list = None,
                  skip_mining: bool = False,
                  skip_oos: bool = False) -> dict:
        run_id = uuid4().hex[:8]
        logger.info(f"{'='*70}")
        logger.info(f"  SPY AI Fund - Cycle {run_id}")
        logger.info(f"{'='*70}")

        # Phase 1: Data inventory + feasibility pre-check
        t0 = time.time()
        ...
        # Phase 1.5: Data feasibility pre-check + auto-fetch
            ...
            for method in meths:
                ...
                if feasibility['auto_fetchable']:
                    logger.info(
                        ...
                        f"auto-fetching {len(feasibility['auto_fetchable'])} missing series..."
                    )
                    fetch_result = self.data_engineer.auto_fetch_missing(
                        feasibility['auto_fetchable']
                    )
                ...
                if feasibility['missing_optional']:
                    logger.info(
                        f"[Phase 1.5] {direction}/{method}: "
                        f"optional missing (ok): {feasibility['missing_optional']}"
                    )
                ...
                logger.info(f"[Phase 1.5] V {direction}/{method}: data feasible")
        ...
        for item in legacy_factors:
            ...
            try:
                ...
                logger.error(f"[Phase 3] Audit error {item['factor_id']}: {e}")
            ...
            if result['verdict'] == 'ACCEPTED':
                ...
                logger.info(
                    f"[Phase 3] V {item['factor_id']}: IS ACCEPTED "
        ...
        logger.info(
            ...
            f"(reflexions: {reflexion_stats['reflexions']}, "
        ...
        if not skip_oos:
            ...
            if factors_for_autopsy:
                logger.info(f"[Phase 3.5] OOS autopsy: {len(factors_for_autopsy)} factors")

                for item in factors_for_autopsy:
                    t0 = time.time()
                    try:
                        result = self.gatekeeper.oos_autopsy(
                            item['factor_id'], item['source_file']
                        )
                    except Exception as e:
                        logger.error(f"[Phase 3.5] Autopsy error {item['factor_id']}: {e}")
                    ...
                    if result['verdict'] == 'ACCEPTED':
                        ...
                        logger.info(
                            f"[Phase 3.5] V {item['factor_id']}: OOS SURVIVED "
                            f"(CondIC={result.get('conditional_ic', 0):+.4f})"
                        )
                    else:
                ...
                logger.info("[Phase 3.5] No factors needing OOS autopsy")
        else:
        ...
        if all_accepted:
            ...
            try:
                ...
                log_agent(run_id, 'ml_synthesizer', 'synthesize',
                          ...
                          duration_sec=t1 - t0, status='error')
        else:
        ...
        report = self._generate_report(
            ...
            all_accepted, feasible_combos, skipped_combos,
        )
        logger.info(f"\n{report['summary']}")
        return report

    def _reflexion_loop(self, run_id, factor_id, direction, method, data_inv):
        ...
        for attempt in range(1 + MAX_REFLEXION_RETRIES):
            ...
            try:
                ...
                logger.error(f"[Reflexion] Audit error {factor_id} (attempt {attempt}): {e}")
                result = {'verdict': 'DEAD', 'reject_reason': str(e)}
            t1 = time.time()
            ...
            if result['verdict'] == 'ACCEPTED':
                logger.info(
                    f"[Reflexion] V {factor_id}: IS ACCEPTED "
                    f"(attempt {attempt}, "
                    f"CondIC={result.get('conditional_ic', 0):+.4f}, "
                    f"HitRate={result.get('hit_rate', 0):.1%})"
                )
        ...
        return {'accepted': False, 'reflexion_count': reflexion_count, 'reflexion_success': False}

    def _build_reflexion_prompt(self, factor_id, audit_result, direction, method):
        ...
        diagnosis_lines = [f"- Rejection reason: {reject_reason}"]
        ...
        if pk_triggered:
            diagnosis_lines.append(
                f"- Overlap PK triggered (Overlap_A={max_overlap_a:.2f}, Overlap_B={max_overlap_b:.2f})"
            )
            if pk_won is False:
        ...
        if 'code' in reject_reason.lower() or 'error' in reject_reason.lower() or 'failed' in reject_reason.lower():
            fix_instructions.append(
                "1. Code execution error: Fix the bug, handle NaN values (.fillna(0.0)), "
                "ensure all column names exist in DataFrame (use data.get('col', pd.Series) to prevent KeyError)."
            )
        if trigger_rate > 0.20:
            fix_instructions.append(
                "2. Trigger rate too high (>20%): Your conditions are too loose or you didn't use .diff(). "
                "Completely change the calculation logic (e.g., use ratio change, first derivative, Z-Score threshold trigger) "
                "to make it a rare pulse. The signal must NOT be non-zero on most trading days!"
            )
        if trigger_rate < 0.001:
            fix_instructions.append(
                "2. Trigger rate near 0%: Your conditions are too strict, almost no trading days trigger. "
                "Relax thresholds or use continuous signal (e.g., tanh compression)."
            )
        if cond_ic <= 0.015 and trigger_rate > 0:
            fix_instructions.append(
                "3. Conditional IC extremely low: Signal direction prediction is insufficient. "
                "Re-examine economic logic, ensure signal direction has causal relationship with SPY price movement. "
                "REMEMBER: For SPY, extreme fear = BUY signal (+1.0), mild fear = bearish (-1.0)."
            )
        if hit_rate < 0.53 and trigger_rate > 0:
            fix_instructions.append(
                "4. Hit Rate too low (<53%): Signal direction accuracy insufficient. "
                "Check if signal direction is correct (bullish for SPY = positive value)."
            )
        if pk_triggered and pk_won is False:
            fix_instructions.append(
                "5. Overlap PK lost: Your logic highly overlaps with existing factors. "
                "ABANDON current approach and find a completely new economic dimension. "
                "Do NOT just tweak parameters (e.g., >2.0 to >2.05), must restructure at logic level!"
            )

        if not fix_instructions:
            fix_instructions.append(
                "1. Re-examine the factor's economic logic, ensure signal has clear causal explanation "
                "specific to US equities (long-bull + mean reversion)."
            )

        anti_phacking = (
            "WARNING: Anti-overfitting - Absolutely forbidden to pass by micro-tuning constant parameters "
            "(e.g., >2.0 to >2.05, or window=20 to window=21). "
            "You MUST restructure at the financial economics logic level!"
        )

        prompt = (
            f"Your previously generated factor code ran successfully, but was rejected by the IS audit.\n\n"
            f"[Failure Diagnosis Report]:\n"
            + "\n".join(diagnosis_lines) + "\n\n"
            f"[Mandatory Fix Instructions]:\n"
            + "\n".join(fix_instructions) + "\n\n"
            f"{anti_phacking}\n\n"
            f"Output the modified complete Python code."
        )

        return prompt

    def _get_legacy_draft_factors(self, exclude_ids):
        exclude_set = set(exclude_ids)
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute(
            "SELECT factor_id, source_file FROM spy_ai_factor_registry "
            "WHERE status = 'draft'"
        )
        ...
        for row in rows:
            if row[0] not in exclude_set:
                factors.append({'factor_id': row[0], 'source_file': row[1]})
        return factors

    def _run_synthesis(self, accepted_factors):
        ...
        core_signal = df['spy_core_signal']
        core_exposure = df['target_exposure'] if 'target_exposure' in df.columns else None
        ...
        result_df = self.ml_synthesizer.synthesize(
            df, core_signal, factor_signals,
        ...
        if not result_df.empty:
            logger.info(
                f"[MLSynthesizer] Synthesis done: "
                f"sat_composite mean={result_df['sat_composite'].mean():.4f}, "
                f"total_score mean={result_df['total_score'].mean():.4f}"
            )

    def _generate_report(self, run_id, new_factor_ids,
                         is_accepted_ids, is_dead_ids,
                         oos_survived_ids, oos_dead_ids,
                         all_accepted,
                         feasible_combos=None, skipped_combos=None):
        lines = [
            f"{'='*60}",
            f"  SPY AI Fund - Cycle Report [{run_id}]",
            f"{'='*60}",
            f"",
        ...
        if oos_dead_ids:
            lines.append("  OOS DEAD (permanently retired):")
            for fid in oos_dead_ids:
                lines.append(f"    - {fid}")

        if all_accepted:
            lines.append("")
            lines.append("  Production Factor Pool (IS+OOS dual-pass):")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='SPY AI Fund Orchestrator')
    parser.add_argument('--mode', choices=['once', 'loop'], default='loop',
                        help='Run mode: once=single, loop=continuous auto-mine (default: loop)')
    parser.add_argument('--interval', type=int, default=None,
                        help=f'Loop interval seconds (default: {AUTO_MINE_INTERVAL_SEC})')
    ...
    parser.add_argument('--direction', type=str, default=None,
                        help='Only mine specified direction (policy_pivot/panic_mean_reversion)')
    parser.add_argument('--method', type=str, default=None,
                        help='Only mine specified method (nonlinear/unstructured)')
    ...
    if args.mode == 'once':
        ...
        logger.info(f"Continuous auto-mine mode | interval: {interval}s | max cycles: {max_cycles or 'infinite'}")
        cycle_count = 0
        while True:
            cycle_count += 1
            logger.info(f"{'#'*70}")
